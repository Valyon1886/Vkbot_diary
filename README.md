...
<!---->
**Примечание:** Вы должны иметь [docker-compose.yml](docker-compose.yml) в корневой папке проекта.
Также, если в строке, которую вывводите в значения переменных окружения, есть знак доллара, то его надо экранировать долларом. Пример: было так - `"fung$gbiobm"`, а надо так - `"fung$$gbiobm"`.
...
services:
  ...
  vkbotdiary:
    ...
    environment:
        ...
        BOT_AWAIT_TIME: 3600
```
Настройка значений переменных окружения аналогична.
