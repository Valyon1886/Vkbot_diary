...
for event in longpoll.listen():
    if event.type == VkEventType.MESSAGE_NEW and event.text and event.to_me:
        ...
        if response == 'начать':
            bot.send_message(vk_session, event.user_id, message='Привет, '
            ...
            if response.upper() in parser.get_schedules()["groups"]:
                users[str(event.user_id)] = response.upper()
                json.dump(users, open("local_files/users_cache.json", "w"))
                bot.send_message(vk_session, event.user_id,
                                 message='Группа сохранена! Номер твоей группы: ' + users[str(event.user_id)])
            else:
                bot.send_message(vk_session, event.user_id, message='Такой группы нет, попробуй еще раз.')
            response = 'Продолжить'
            keyboard = bot.create_menu(response)

        elif response == 'расписание':
            bot.send_message(vk_session, event.user_id, message='Выбери возможность',
                             keyboard=keyboard)

        elif re.search(r'(на [а-я]+( [а-я]+)?)|(какая [а-я]{6})', response):
            try:
                bot.schedule_menu(response, vk_session, event.user_id, parser.get_schedules(), users)
            except KeyError:
                bot.send_message(vk_session, event.user_id, message='Вы не ввели группу.\n Формат ввода: ИКБО-09-19')
            response = 'Продолжить'
            keyboard = bot.create_menu(response)

        else:
            bot.send_message(vk_session, event.user_id, message='Я не знаю такой команды')
            response = 'Продолжить'
            keyboard = bot.create_menu(response)

        if response == 'Продолжить':
            bot.send_message(vk_session, event.user_id, message='Что хочешь посмотреть?', keyboard=keyboard)
